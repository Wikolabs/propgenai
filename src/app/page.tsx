...
export default function DemoPage() {
  ...
  const t = lang === "fr" ? {
    back: "Retour", title: "Demo", sub: PRODUCT + " — proposition commerciale IA",
    desc: "Collez vos notes de discovery commercial. L'agent genere une proposition structuree (besoin, scope, prix, ROI, plan 30/60/90) prete a etre signee. Aucune connexion DocuSign/Pandadoc reelle — c'est un POC qui demontre la logique de production.",
    inputLabel: "Notes de discovery", placeholder: "ex : Call avec Marc, VP RevOps chez TechCorp. Probleme : 4 SDR perdent 30% temps sur prospection manuelle. Deja HubSpot, evaluent Outreach. Budget mentionne : ~50k EUR/an. Decideurs : VP RevOps + CRO. Timeline : signature avant fin T1.",
    generate: "Generer la proposition", generating: "Redaction en cours...",
    briefTitle: "Proposition commerciale", emptyHint: "La proposition apparaitra ici.",
    sendDocusign: "Envoyer DocuSign", exportPdf: "Export PDF", saveSalesforce: "Sauver dans Salesforce",
    sentDocusignMock: "Proposition envoyee via DocuSign (mode demo, pas de signature reelle)",
    exportedPdfMock: "PDF genere et telecharge (mode demo, fichier simule)",
    savedSfMock: "Opportunite mise a jour dans Salesforce (mode demo, pas de sync CRM reelle)",
    fallback: "Mode statique : la cle LLM sera ajoutee au prochain deploiement.",
    poweredBy: "Modele :",
    note: "DEMO POC — aucune connexion reelle DocuSign, Salesforce, Stripe. L'IA invente la proposition pour la demonstration.",
  } : {
    back: "Back", title: "Demo", sub: PRODUCT + " — AI commercial proposal",
    desc: "Paste your sales discovery notes. The agent generates a structured proposal (need, scope, price, ROI, 30/60/90 plan) ready to be signed. No real DocuSign/Pandadoc connection — this is a POC showing production logic.",
    inputLabel: "Discovery notes", placeholder: "e.g. Call with Marc, VP RevOps at TechCorp. Problem: 4 SDRs lose 30% of time on manual prospecting. Already HubSpot, evaluating Outreach. Budget mentioned: ~50k EUR/year. Deciders: VP RevOps + CRO. Timeline: sign before end of Q1.",
    generate: "Generate proposal", generating: "Drafting...",
    briefTitle: "Commercial proposal", emptyHint: "The proposal will appear here.",
    sendDocusign: "Send via DocuSign", exportPdf: "Export PDF", saveSalesforce: "Save to Salesforce",
    sentDocusignMock: "Proposal sent via DocuSign (demo mode, no real signature)",
    exportedPdfMock: "PDF generated and downloaded (demo mode, simulated file)",
    savedSfMock: "Opportunity updated in Salesforce (demo mode, no real CRM sync)",
    fallback: "Static mode: LLM key will be added at next deploy.",
    poweredBy: "Model:",
    note: "DEMO POC — no real DocuSign, Salesforce, Stripe connection. The AI invents the proposal for demonstration.",
  };

  async function generate() {
    setError(""); setBrief(""); setModel(""); setStaticMode(false);
    if (!notes.trim()) {
      setError(lang === "fr" ? "Collez les notes de discovery." : "Paste discovery notes.");
      return;
    }
    setLoading(true);
    try {
      const r = await fetch("/offers/propgenai/demo/api/demo", {
        method: "POST",
      ...
      <main style={{ flex: 1, padding: "32px", maxWidth: 1200, margin: "0 auto", width: "100%" }}>
        ...
        <div className="demo-grid" style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: 24 }}>
          <section style={{ background: PAL.surface, border: `1px solid ${PAL.border}`, borderRadius: 16, padding: 22 }}>
            ...
            <textarea className="wk-textarea" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={t.placeholder} />
          ...
          <section style={{ background: PAL.bg2, border: `1px solid ${PAL.border}`, borderRadius: 16, padding: 22, minHeight: 420, display: "flex", flexDirection: "column" }}>
            ...
            {brief && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 18, paddingTop: 18, borderTop: `1px solid ${PAL.border}` }}>
                <button className="wk-btn-ghost" onClick={() => showToast(t.sentDocusignMock)}>✍️ {t.sendDocusign}</button>
                <button className="wk-btn-ghost" onClick={() => showToast(t.exportedPdfMock)}>📄 {t.exportPdf}</button>
                <button className="wk-btn-ghost" onClick={() => showToast(t.savedSfMock)}>☁️ {t.saveSalesforce}</button>
              </div>
            )}
